...
async def generate_question_for_pair_async(
    pair: ChunkPair,
    client: AsyncOpenAI,
    ...
    """Async version of generate_question_for_pair.
...
{{
  ...
  "difficulty": "easy|medium|hard"
    ...
    try:
        ...
        # Validate if requested
        if validate:
            is_valid = await validate_question_async(question, client, semaphore, model)
            question.validation_passed = is_valid
            if not is_valid:
                return None
...
async def validate_question_async(
    question: MultiHopQuestion,
    client: AsyncOpenAI,
    semaphore: asyncio.Semaphore,
    model: str = "gpt-5.2",
) -> bool:
    """Async version of validate_question.
...
async def generate_multihop_questions_async(
    ...
    """Async version of generate_multihop_questions.

    Generates questions concurrently with rate limiting.
    ...
    pairs_to_process = chunk_pairs[:max_attempts]

    print(f"Generating {len(pairs_to_process)} questions concurrently...")

    # Create tasks for all pairs
    tasks = [
        generate_question_for_pair_async(pair, client, semaphore, model, validate)
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            ...
            print(f"  Generated: {result.question[:60]}...")
            if max_questions and len(questions) >= max_questions:
                break

    print(f"\nGenerated {len(questions)} valid questions from {len(pairs_to_process)} attempts")
    return questions
